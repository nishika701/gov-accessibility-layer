...
app = FastAPI(
    ...
    description=(
        ...
        "Supports S3 storage, Textract OCR extraction, SageMaker LLM simplification, "
...
def root_info():
    ...
    return {
        ...
        "endpoints": {
            ...
            "POST /explain": "Generate citizen-friendly explanation via Amazon SageMaker",
            "POST /question": "Generate document understanding question via Amazon SageMaker",
            "POST /check-answer": "Verify citizen answer against document context via SageMaker",
        }
    }
...
def explain_text(request: ExplainRequest):
    """
    POST /explain — Sends extracted text to SageMaker and returns simplified explanation.
    Falls back gracefully to local Ollama or structured mock intelligence.
    """
    ...
    try:
        result = sagemaker_service.explain_document(
            text=request.text,
...
def generate_question(request: QuestionRequest):
    """
    POST /question — Generates a document-specific understanding question using SageMaker.
    ...
    try:
        result = sagemaker_service.generate_question(
...
def check_citizen_answer(request: CheckAnswerRequest):
    """
    POST /check-answer — Sends the user's answer + document context to SageMaker
    ...
    try:
        result = sagemaker_service.check_answer(
